...
//   return element;
// })
let evenNumbers = mixedNumbers.filter((element, index, wholearray) => {if(element % 2 === 0) return element})
...
//   return element * 1.07
// })
let postTaxPrices = prices.map((element, index, array) => element * 1.07)
  //   return element * 1.07
...
let myStrongest = monstersInYourPocket.filter(function(element, index, array) {
  if (monstersInYourPocket[index].CP > 200) {
    return monstersInYourPocket[index]
  }
})
...
// let orderTotals = [];
// let counter = 0;
// orderTotals = orders.map(function() {
//   let o = orders[counter].price * (1 + orders[counter].tax);   
// })

// orderTotals = orders.map(function(element, index, array)
// {
//   return element.price * (1 + orders[index].tax);
// });
